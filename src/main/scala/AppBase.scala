...
trait AppBase[User] extends WabaseAppCompat[User] with Authorization[User] with Loggable with QuereaseProvider with DbAccessProvider with I18n with RowWriters {
  ...
  */
  ...
  def save(viewName: String, obj: Map[String, Any], params: Map[String, Any] = Map(), emptyStringsToNull: Boolean = true)(
    implicit user: User, state: ApplicationState, timeoutSeconds: QueryTimeout,
      poolName: PoolName = ConnectionPools.key(viewDef(viewName).db)) = {
    val instance = qio.fill[Dto](qe.toCompatibleMap(obj, viewDef(viewName)))(Manifest.classType(viewNameToClassMap(viewName)))
      ...
      ListMap(
        "filter" -> viewNameToFilterMetadata(viewName).map(f => ListMap(
          ...
          "label" -> Option(f.label).map(fl => s"${fl.fieldName}${
              Option(fl.filterName).map(fn => s" (${translate(fn)(Option(state)
                .map(_.locale).getOrElse(Locale.forLanguageTag("en")))})").getOrElse("")}"
            ).orNull,
          "enum" -> Option(f)
            .map(_.enum_)
            .filter(_ != null)
