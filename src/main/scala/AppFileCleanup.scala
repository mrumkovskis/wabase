  ...
  protected def fileInfoCleanupSelectStatement(fs: AppFileStreamerConfig) = {
    ...
    val selectStatement =
      if (tableMetadataWithFileInfo.isEmpty) {
        s"${fs.file_info_table} fi[fi.upload_time < sql(\"$ageCheckSql\")]{fi.id}"
      } else {
        val unionSubquery = tableMetadataWithFileInfo.map {
          case (table, col) => s"select $col id from $table"
        }.mkString(" union all ")
        s"${fs.file_info_table} fi[fi.upload_time < sql(\"$ageCheckSql\") & !exists(sql(\"select 1 from ($unionSubquery) all_refs where all_refs.id = fi.id\"))]{fi.id}"
      }
    selectStatement
  }
