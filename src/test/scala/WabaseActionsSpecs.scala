...
class WabaseActionsSpecs extends AsyncFlatSpec with Matchers with TestQuereaseInitializer with AsyncFlatSpecLike
  ...
  protected def doJob(jobName: String, params: Map[String, Any]): Future[Any] = {
    wabaseScheduler.doJob(jobName, params)
  }
  ...
  it should "compile metadata" in {
    ...
    app.qe.asInstanceOf[compiling.WabaseViewCompiler].compileAllQueries(Set(), true, msgs ::= _)
  ...
  it should "do invocations" in {
    for {
      ...
      t14 <- doAction("update", "invocation_test_4", Map()).map {
        _ shouldBe StringResult("null null")
      }
      t15 <- doAction("count", "invocation_test_4", Map()).map {
        _ shouldBe NoResult
      }
    } yield {
      t2
  ...
  it should "evaluate if" in {
    for {
      ...
      t11 <- doAction("insert", "if_test_2", Map("value" -> "insert")).map {
        _ shouldBe StringResult("I")
      }
      t12 <- doAction("insert", "if_test_2", Map("value" -> "x")).map {
        _ shouldBe StringResult("NI")
      }
      t13 <- doAction("update", "if_test_2", Map("value" -> "update")).map {
        _ shouldBe StringResult("U")
      }
      t14 <- doAction("update", "if_test_2", Map("value" -> "x")).map {
        _ shouldBe StringResult("NU")
      }
      t15 <- doAction("delete", "if_test_2", Map("value" -> "delete")).map {
        _ shouldBe StringResult("D")
      }
      ...
      _ <- doAction("count", "if_test_2", Map("value" -> "V")).map {
        _ shouldBe StringResult("V")
      }
      _ <- doAction("count", "if_test_2", Map("value" -> null)).map {
        _ shouldBe NoResult
      }
    } yield {
      t4
  ...
  it should "do wabase routes and return results" in {
    ...
    */
    ...
    Get("/new/invocation_test_3") ~> route ~> check {
      val r = entityAs[String]
